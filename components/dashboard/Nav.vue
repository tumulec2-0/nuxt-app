...
const userEmail = ref('');
const userName = ref('Jan Kowalski');  // Default name or fetched from some API

onMounted(() => {
    const userStoredEmail = localStorage.getItem('user-email');
    if (userStoredEmail) {
        userEmail.value = userStoredEmail;
    } else {
        navigateTo('/login');
    }
});
